...
int n, m;
int long long tree[100001 << 2];
int long long lazy[100001 << 2];
...
void update(const int L, const int R, const int x, int l = 1, int r = n, int root = 1){
    if(L <= l && R >= r){
        tree[root] += (int long long)(r - l + 1) * x;
        lazy[root] += x;
        return;
    }
    PushDown(root, r - l + 1);
...
int long long query(const int L, const int R, int l = 1, int r = n, int root = 1){
    if(L <= l && R >= r)
        return tree[root];
    PushDown(root, r - l + 1);
